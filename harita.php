...
$tsHandle = TeamSpeak3::factory("serverquery://".$cfg->TsBilgiler->user.":".$cfg->TsBilgiler->pass."@".$cfg->TsBilgiler->host.":".$cfg->TsBilgiler->qport."/?server_port=".$cfg->TsBilgiler->port."&nickname=".urlencode($cfg->TsBilgiler->nickname));
$clientList = $tsHandle->clientList();

$countryCodes = array();

foreach ($clientList as $client){
  if($client['client_type'] == 0){
    array_push($countryCodes, $client['client_country']);
  }
}
...
foreach ($countryCodes as $country) {
  $countryName = countryCodeToCountry($country);
  if($countryName != ''){
    if(array_key_exists($countryName, $countryStats)){
      $countryStats[$countryName]++;
    } else {
      $countryStats[$countryName] = 1;
    }
  }
}
?>
<html>
        ...
        var data = google.visualization.arrayToDataTable([
          ...
          <?php
            foreach ($countryStats as $key => $value){
              if($key == end(array_keys($countryStats))){
                echo '[\''.$key.'\', '.$value.']';
              } else {
                echo '[\''.$key.'\', '.$value.'], ';
              }
  ...
  <body>
    <div id="regions_div" style="width: 900px; height: 500px; border: 1px solid rgb(226, 226, 226); border-top-width: 0; border-radius: 4px;"></div>
  </body>
</html>
